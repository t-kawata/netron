...
// Config is the config struct for the node and the default Netron services
...
func (c Config) Validate() error {
	if c.NetworkConfig == "" &&
		c.NetworkToken == "" {
		return fmt.Errorf("NETRONCONFIG or NETRONTOKEN not supplied. At least a config file is required")
	}
	return nil
}
...
func (c Config) ToOpts(l *logger.Logger) ([]node.Option, []vpn.Option, error) {
	...
	libp2pOpts := []libp2p.Option{libp2p.UserAgent("netron")}
	...
	if ledgerState != "" {
		opts = append(opts, node.WithStore(blockchain.NewDiskStoreWithLimit(
			diskv.New(diskv.Options{BasePath: ledgerState, CacheSizeMax: uint64(50)}),
			consts.MAX_BLOCKS,
		)))
	} else {
		opts = append(opts, node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)))
	}
