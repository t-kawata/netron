...
var _ = Describe("trustzone", func() {
	...
	Context("ECDSA auth", func() {
		It("authorize nodes", func() {
			...
			permStore := blockchain.NewMemoryStore(consts.MAX_BLOCKS)
			...
			e2, _ := node.New(
				...
				//	node.WithPeerGater(pg),
				node.WithDiscoveryInterval(10*time.Second),
				node.FromBase64(true, true, token, nil, nil), node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), ll)
