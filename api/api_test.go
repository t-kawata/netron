...
import (
	...
	. "github.com/mudler/netron/api"
	client "github.com/mudler/netron/api/client"
...
var _ = Describe("API", func() {
	...
	Context("Binds on socket", func() {
		It("sets data to the API", func() {
			...
			e, _ := node.New(node.FromBase64(true, true, token, nil, nil), node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
			e.Start(ctx)

			e2, _ := node.New(node.FromBase64(true, true, token, nil, nil), node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
			e2.Start(ctx)
