...
var _ = Describe("File services", func() {
	...
	e2, _ := node.New(
		...
		node.FromBase64(true, true, token, nil, nil), node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
	...
	Context("File sharing", func() {
		It("sends and receive files between two nodes", func() {
			...
			os.WriteFile(f.Name(), []byte("testfile"), os.ModePerm)

			// First node expose a file
			opts, err := ShareFile(logg, 10*time.Second, fileUUID, f.Name())
			Expect(err).ToNot(HaveOccurred())

			opts = append(opts, node.FromBase64(true, true, token, nil, nil), node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
			e, _ := node.New(opts...)
