...
var _ = Describe("Expose services", func() {
	...
	Context("Service sharing", func() {
		PIt("expose services and can connect to them", func() {
			...
			opts = append(opts, node.FromBase64(true, true, token, nil, nil), node.WithDiscoveryInterval(10*time.Second), node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
			e, _ := node.New(opts...)
