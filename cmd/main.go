...
import (
	...
	netron "github.com/mudler/netron/pkg/node"
	"github.com/mudler/netron/pkg/services"
	"github.com/mudler/netron/pkg/vpn"
	"github.com/urfave/cli/v2"
)

const Copyright string = `Netron Copyright (C) ShyMe Inc. Toshimi Kawata `
...
func MainFlags() []cli.Flag {
	...
	return append([]cli.Flag{
		...
		&cli.StringFlag{
			Name:    "lease-dir",
			Value:   filepath.Join(basedir, ".netron", "leases"),
...
func Main() func(c *cli.Context) error {
	return func(c *cli.Context) error {
		if c.Bool("g") {
			...
			newData := netron.GenerateNewConnectionData(c.Int("key-otp-interval"))
			if c.Bool("b") {
		...
		e, err := netron.New(append(o, opts...)...)
		if err != nil {
			return err
		}
