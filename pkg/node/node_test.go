...
import (
	...
	"github.com/mudler/netron/consts"
...
var _ = Describe("Node", func() {
	...
	Context("Configuration", func() {
		It("fails if is not valid", func() {
			_, err := New(FromBase64(true, true, "  ", nil, nil), WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
			Expect(err).To(HaveOccurred())
			_, err = New(FromBase64(true, true, token, nil, nil), WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
	...
	Context("Connection", func() {
		It("see each other node ID", func() {
			...
			e, _ := New(FromBase64(true, true, token, nil, nil), WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
			e2, _ := New(FromBase64(true, true, token, nil, nil), WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), l)
		...
		It("nodes can write to the ledger", func() {
			...
			e, _ := New(FromBase64(true, true, token, nil, nil), WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), WithDiscoveryInterval(10*time.Second), l)
			e2, _ := New(FromBase64(true, true, token, nil, nil), WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)), WithDiscoveryInterval(10*time.Second), l)
	...
	Context("connection gater", func() {
		It("blacklists", func() {
			...
			e, _ := New(
				...
				WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)),
				l,
			)
