...
// WithAPIAddress sets the Netron API address
...
func (k *Node) prepare() error {
	...
	if k.networkToken == "" {
		...
		newData := netron.GenerateNewConnectionData()
		...
		os.WriteFile(k.tokenFile, []byte(token), os.ModePerm)
		k.networkToken = token
	}
	...
	if k.client == nil {
		k.client = NewClient("Node",
			netronClient.NewClient(netronClient.WithHost(fmt.Sprintf("http://%s", k.apiAddress))))
	}
	return nil
}
