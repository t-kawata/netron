...
var _ = Describe("Alive service", func() {
	...
	Context("Aliveness check", func() {
		It("detect both nodes alive after a while", func() {
			...
			defer cancel()
			e2, _ := node.New(append(opts, node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)))...)
			e1, _ := node.New(append(opts, node.WithStore(blockchain.NewMemoryStore(consts.MAX_BLOCKS)))...)
